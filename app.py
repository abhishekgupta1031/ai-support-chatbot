...
def init_db():
    ...
    if "intent" not in columns:
        cursor.execute("ALTER TABLE chat_logs ADD COLUMN intent TEXT")
...
def save_chat(user_message, bot_response, intent=None):
    try:
        conn = get_db_connection()
        conn.execute(
            "INSERT INTO chat_logs (user_message, bot_response, intent) VALUES (?, ?, ?)",
...
def load_faqs():
    """Safely load FAQ list from either {'faqs': [...]} or [...] JSON format."""
    try:
...
def save_faqs(faqs):
    """Always save FAQs using the standard {'faqs': [...]} format."""
    try:
        os.makedirs(os.path.dirname(FAQ_FILE), exist_ok=True)
...
def chat():
    ...
    try:
        ...
        save_chat(user_message, bot_response, intent)
...
def admin():
    ...
    today_chats = conn.execute(
        "SELECT COUNT(*) AS count FROM chat_logs WHERE DATE(timestamp) = ?",
        (today,)
    ).fetchone()["count"]

    total_responses = conn.execute(
        "SELECT COUNT(*) FROM chat_logs WHERE bot_response IS NOT NULL"
    ).fetchone()[0]
    ...
    intent_counts = [row["count"] for row in intent_rows]

    conn.close()

    # IMPORTANT: FAQ list is loaded safely even when faq.json is a dictionary.
    faqs = load_faqs()
...
def add_faq():
    ...
    # Keep FAQ usable even if the keywords field is empty.
    faqs = load_faqs()
    ...
    if request.method == "POST":
        ...
        if not ADMIN_EMAIL:
            ...
            return render_template(
                "forgot_password.html",
                message=None,
                error="Password reset email is not configured yet. Please configure ADMIN_EMAIL in environment variables."
            )
        ...
        if send_reset_email(username, reset_link):
            message = "A password reset link has been sent to the registered admin email."
        else:
            error = "Unable to send the reset email. Please check SMTP environment variables."
    ...
    if request.method == "POST":
        ...
        if len(new_password) < 8:
            ...
            conn.execute(
                ...
                (password_hash, reset_record["username"])
            )
